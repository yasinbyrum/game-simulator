...
function renderChestConfigViz() {
    ...
    let h = `<div style="margin-bottom:15px; padding-bottom:10px; display:flex; align-items:center;">${chestImgHTML}<div><div style="font-weight:bold; color:var(--gold); margin-bottom:5px;">💰 Gold Drops</div><div style="display:flex; gap:10px;">
        ...
        ${[3, 4, 5].map(b => `<div class="sim-input-group"><label>B${b}</label><input type="number" class="edit" value="${c.gold['b' + b]}" onchange="updateVal('chestConfigs[\`' + currentChestTab + '\`].gold.b${b}',this.value)" style="width:50px; font-size:1.15rem;"></div>`).join('')}</div></div></div>`;

    h += `<div style="font-weight:bold; color:#a855f7;">🃏 Card Amounts</div><table style="width:100%; font-size:1.15rem;"><thead><tr><th>Rarity</th><th>B1-B2</th><th>B3</th><th>B4</th><th>B5</th></tr></thead><tbody>
        ${['Rookie', 'Pro', 'Champion', 'Legendary'].map(r => `<tr><td>${r}</td>
            <td><input type="number" class="edit" value="${c.amounts[r]['b1']}" onchange="updateB1B2('chestConfigs[\`' + currentChestTab + '\`].amounts.${r}',this.value)" style="width:50px; font-size:1.15rem;"></td>
            ${[3, 4, 5].map(b => `<td><input type="number" class="edit" value="${c.amounts[r]['b' + b]}" onchange="updateVal('chestConfigs[\`' + currentChestTab + '\`].amounts.${r}.b${b}',this.value)" style="width:50px; font-size:1.15rem;"></td>`).join('')}
        </tr>`).join('')}</tbody></table>`;

    h += `<div style="font-weight:bold; color:var(--accent); margin-top:15px;">🎲 Slot Probabilities (%)</div><table style="width:100%; font-size:1.15rem;"><thead><tr><th>Slot</th><th>B1-B2 (R-P-C-L)</th><th>B3 (R-P-C-L)</th><th>B4 (R-P-C-L)</th><th>B5 (R-P-C-L)</th></tr></thead><tbody>`;
    c.slotProbs.forEach((s, i) => {
        h += `<tr><td style="font-size:1.15rem;">Slot ${s.id}</td>
            <td><div style="display:flex; gap:5px;">
                <input class="edit" value="${s['b1'].R}" onchange="updateB1B2('chestConfigs[\`' + currentChestTab + '\`].slotProbs[${i}].',this.value, 'R')" style="width:50px; font-size:1.2rem; text-align:center;" title="R">
                <input class="edit" value="${s['b1'].P}" onchange="updateB1B2('chestConfigs[\`' + currentChestTab + '\`].slotProbs[${i}].',this.value, 'P')" style="width:50px; font-size:1.2rem; text-align:center;" title="P">
                <input class="edit" value="${s['b1'].C}" onchange="updateB1B2('chestConfigs[\`' + currentChestTab + '\`].slotProbs[${i}].',this.value, 'C')" style="width:50px; font-size:1.2rem; text-align:center;" title="C">
                <input class="edit" value="${s['b1'].L}" onchange="updateB1B2('chestConfigs[\`' + currentChestTab + '\`].slotProbs[${i}].',this.value, 'L')" style="width:50px; font-size:1.2rem; text-align:center;" title="L">
            </div></td>
            ${[3, 4, 5].map(b => `<td><div style="display:flex; gap:5px;"><input class="edit" value="${s['b' + b].R}" onchange="updateVal('chestConfigs[\`' + currentChestTab + '\`].slotProbs[${i}].b${b}.R',this.value)" style="width:50px; font-size:1.2rem; text-align:center;" title="R"><input class="edit" value="${s['b' + b].P}" onchange="updateVal('chestConfigs[\`' + currentChestTab + '\`].slotProbs[${i}].b${b}.P',this.value)" style="width:50px; font-size:1.2rem; text-align:center;" title="P"><input class="edit" value="${s['b' + b].C}" onchange="updateVal('chestConfigs[\`' + currentChestTab + '\`].slotProbs[${i}].b${b}.C',this.value)" style="width:50px; font-size:1.2rem; text-align:center;" title="C"><input class="edit" value="${s['b' + b].L}" onchange="updateVal('chestConfigs[\`' + currentChestTab + '\`].slotProbs[${i}].b${b}.L',this.value)" style="width:50px; font-size:1.2rem; text-align:center;" title="L"></div></td>`).join('')}
        </tr>`;
    });
